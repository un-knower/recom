...
object UserSimilarStreaming {
  ...
  private val logger = Logger.getLogger(ViewsStreaming.getClass)
  private val pro = new Properties()
  pro.load(this.getClass.getResourceAsStream("/uat_streaming.properties"))
  private val userSimilarCheckDir = pro.getProperty("userSimilarCheckDir")
  ...
  private val userPath = pro.getProperty("userPath")
    ...
    if (bTFVectorRDD == null) {
      synchronized {
        if (bTFVectorRDD == null) {
          ...
          val path = s"${userPath}/dt=${yesterday}"
          val textRDD = sc.textFile(path)
          bTFVectorRDD = sc.broadcast(getUserVectorRDD(textRDD))
        }
      }
    }
    bTFVectorRDD
  ...
  def main(args: Array[String]): Unit = {
    val ssc = StreamingContext.getActiveOrCreate(userSimilarCheckDir, functionToCreateContext)
    ssc.start()
    logger.info("**********新认证通过用户计算相似度实时流处理程序开始执行***********")
  ...
  def functionToCreateContext(): StreamingContext = {
    val conf = new SparkConf().setAppName("uat_userSimilarStreaming").set("spark.streaming.stopGracefullyOnShutdown", "true").setMaster("yarn-client")
    //初始化Streamingcontext设置为批次为一分钟
    val sc = new SparkContext(conf)
    val ssc = new StreamingContext(sc, Seconds(batchDuration.toInt))
    ssc.sparkContext.setLogLevel("WARN")
    //为ssc设置checkpoint
    ssc.checkpoint(userSimilarCheckDir)
    //读取新增内容topic的数据
    val pro = new Properties()
    pro.load(this.getClass.getResourceAsStream("/uat_consumer.properties"))
    val topicSet = pro.getProperty("userSimilarTopic").split(",").toSet
    val brokers = pro.getProperty("brokers")
    val groupId = pro.getProperty("userSimilarGroupId")
              ...
              if (info.length != 16) {
                ...
                info(10) match {
                  case "1" => {
                    ...
                    val status = redis.hget("US:STATUS", s"${id}")
                    if(status == null){
                      redis.hset("US:STATUS", s"${id}", "1:1")
                    }else{
                      val info = status.split(":")
                      if(info.length == 2){
                        redis.hset("US:STATUS", s"${id}", s"2:${info(1)}")
                      }
                    }
//                    redis.close()
                  }
                  case _ => logger.warn("审核未通过")
                }
              }
            case _ => logger.warn("doctorCheck格式不匹配")
          }
          (id, tf)
        })
      }).filter(_._2!=null)
      ...
      //      val idf = new IDF().fit(userRDD.value.values)
      val idfRDD1 = tfRDD.mapValues(v => idfModel.transform(v))
      //      val idfRDD2 = userRDD.value.mapValues(v => idf.transform(v))
      //广播一份tf-idf向量集
      ...
      docSims.foreachPartition(partition=>{
        val redis =  RedisCluster.cluster
        partition.foreach(x=>{
          redis.rpush(s"US:${x._1}", s"{'id': '${x._2}', 's': ${x._3}}")
        })
//        redis.close()
      })
  ...
  def getUserVectorRDD(rdd:RDD[String]): RDD[(String, Vector)] = {
